import { AreaEffectConfig, AreaEffectType, DEFAULT_FLOW_RULES, EffectEngineEvent, EffectRuntimeState, EffectScope, FlowEvent, FlowRules, GemEffectConfig, GemEffectType, LinkDirection, LinkEffectConfig, LinkEffectType, ResolvedEffect } from "./effects.models";
...
export class EffectFlowEngine {
  createRuntime(effects: readonly ResolvedEffect[]): EffectRuntimeState {
    ...
    return { wallRemainingStrength, iceRemainingStrength, shieldRemainingStrength, timerRemainingTurns, completedTimerIds: [], expiredTimerIds: [], turn: 0 };
  }
  ...
  resolve(values: readonly number[], effects: readonly ResolvedEffect[], runtime: EffectRuntimeState, inputs: readonly EffectFlowInput[], overrides?: Partial<FlowRules>, turn?: number, resolvedGemIds: readonly string[] = []): EffectFlowResult {
    ...
    const nextRuntime: EffectRuntimeState = { wallRemainingStrength: { ...(runtime.wallRemainingStrength ?? {}) }, iceRemainingStrength: { ...(runtime.iceRemainingStrength ?? {}) }, shieldRemainingStrength: { ...(runtime.shieldRemainingStrength ?? {}) }, timerRemainingTurns: { ...(runtime.timerRemainingTurns ?? {}) }, completedTimerIds: [...(runtime.completedTimerIds ?? [])], expiredTimerIds: [...(runtime.expiredTimerIds ?? [])], turn: turn ?? (runtime.turn ?? 0) + 1 };
    const events: EffectEngineEvent[] = []; const triggeredBombs = new Set<string>(); const terminalGems = new Set(resolvedGemIds); const gemIndex = new Map<string, number>(); values.forEach((_, index) => gemIndex.set(`target-${index}`, index));
    const gemEffects = new Map<string, ResolvedEffect[]>(); const links = effects.filter((effect): effect is ResolvedEffect & { config: LinkEffectConfig } => effect.config.scope === EffectScope.LINK); const bombs = effects.filter((effect): effect is ResolvedEffect & { config: AreaEffectConfig } => effect.config.scope === EffectScope.AREA && effect.config.type === AreaEffectType.BOMB);
    ...
    while (queue.length) {
      ...
      for (const arrival of arrivals) {
        ...
        for (const link of links) { if (!this.isLinkActive(link, gemIndex, terminalGems)) continue; const target = this.followLink(link, arrival.flow.currentGemId); if (!target || arrival.flow.visitedLinks.has(link.id)) continue; const propagated = this.transformLink(link.config, arrival.value); const visitedLinks = new Set(arrival.flow.visitedLinks); visitedLinks.add(link.id); const id = `${arrival.flow.id}>${link.id}`; queue.push({ id, rootFlowId: arrival.flow.rootFlowId, originGemId: arrival.flow.originGemId, currentGemId: target, value: propagated, generation: generation + 1, sourceType: "PROPAGATED", visitedLinks }); events.push({ type: "FLOW_PROPAGATED", flowId: id, gemId: target, linkId: link.id, value: propagated, generation: generation + 1 }); }
      }
      for (const bomb of bombs) {
        if (bomb.target.type !== EffectScope.AREA) continue; const sourceIndex = gemIndex.get(bomb.target.sourceGem.id); if (triggeredBombs.has(bomb.id) || sourceIndex === undefined || values[sourceIndex] === 0 || nextValues[sourceIndex] !== 0) continue;
        triggeredBombs.add(bomb.id); const strength = Math.abs(bomb.config.strength ?? 1); const radius = bomb.config.radius ?? 1; events.push({ type: "BOMB_TRIGGERED", gemId: bomb.target.sourceGem.id, generation });
        for (let distance = 1; distance <= radius; distance += 1) for (const neighbour of [((sourceIndex - distance) + values.length) % values.length, (sourceIndex + distance) % values.length]) { const id = `area-${bomb.id}-${generation}-${neighbour}`; queue.push({ id, rootFlowId: id, originGemId: bomb.target.sourceGem.id, currentGemId: `target-${neighbour}`, value: -strength, generation: generation + 1, sourceType: "AREA", visitedLinks: new Set<string>() }); events.push({ type: "AREA_TRIGGERED", flowId: id, gemId: `target-${neighbour}`, value: -strength, generation: generation + 1 }); }
      }
    }
    this.applyTurnEnd(nextValues, effects, nextRuntime, events); return { values: nextValues, runtime: nextRuntime, events };
  }
  ...
  private applyTurnEnd(values: number[], effects: readonly ResolvedEffect[], runtime: EffectRuntimeState, events: EffectEngineEvent[]): void {
  ...
  private isLinkActive(effect: ResolvedEffect & { config: LinkEffectConfig }, gemIndex: ReadonlyMap<string, number>, terminalGems: ReadonlySet<string>): boolean { if (effect.target.type !== EffectScope.LINK) return false; const from = gemIndex.get(effect.target.fromGem.id); const to = gemIndex.get(effect.target.toGem.id); return from !== undefined && to !== undefined && !terminalGems.has(effect.target.fromGem.id) && !terminalGems.has(effect.target.toGem.id); }
