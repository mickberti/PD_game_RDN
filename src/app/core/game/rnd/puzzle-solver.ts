...
const keyOf = (state: PuzzleState): string => JSON.stringify([state.rotation, state.outerValues, state.queueCursors, state.consumedSpecialOperatorIndexes, state.phaseCursor, state.effectRuntime?.timerRemainingTurns, state.effectRuntime?.expiredTimerIds]);
...
export const solvePuzzle = (level: LevelDefinition, options: PuzzleSolveOptions): PuzzleSolveResult => {
  ...
  while (queue.length) {
    ...
    if (hasPuzzleFailed(level, current.state)) continue;
