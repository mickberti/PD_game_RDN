...
export interface PuzzleGenerationMetadata { seed: number; generatorVersion: string; balanceVersion?: string; difficulty: "EASY" | "NORMAL" | "HARD" | "EXPERT"; estimatedMinimumSolutionLength: number; /** Canonical replayed solution; this is the official three-star impulse target. */ officialSolutionImpulses?: number; officialSolutionRotationSteps?: number; branchingFactor: number; featureFlags: readonly string[]; generationStats?: { elapsedMs: number; /** Actual candidate boards evaluated before the accepted solution. */ structureAttempts: number; /** Actual replay/calibration attempts evaluated before the accepted solution. */ solutionAttempts?: number; /** Legacy name retained so historical catalogues remain readable. */ calibrationAttempts: number; /** Configured board-seed budget for one fallback stage. */ structureAttemptsBeforeScaling?: number; /** Configured replay budget for one candidate board. */ solutionAttemptsBeforeScaling?: number; totalComplexity: number; failureReasons: readonly string[]; }; }
