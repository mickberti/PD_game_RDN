...
import type { EffectEngineEvent, EffectRuntimeState, ElementalAffinity } from "./effects/effects.models";
...
export type OperationRejectedReason = "NO_OPERATOR" | "TARGET_ALREADY_RESOLVED" | "DIVIDE_BY_TWO_CONSUMED" | "DIVIDE_BY_TWO_REQUIRES_NON_ZERO_EVEN_INTEGER" | "DIVIDE_BY_THREE_CONSUMED" | "DIVIDE_BY_THREE_REQUIRES_NON_ZERO_MULTIPLE_OF_THREE" | "SPECIAL_OPERATOR_CONSUMED" | "RESULT_OUT_OF_RANGE" | "COLOR_MISMATCH" | "CHAIN_LOCKED";
...
export interface PuzzleGenerationDiagnosticContext { phase: "combination-filter" | "calibration"; stage: number; configuration: number; placement: number; variation: number; presets: readonly string[]; }
export interface PuzzleGenerationDiagnostic { code: string; count: number; first: PuzzleGenerationDiagnosticContext; last: PuzzleGenerationDiagnosticContext; }
export interface PuzzleGenerationStageStats { stage: number; configurations: number; placementVariants: number; candidateVariants: number; policyRejectedCandidates: number; evaluatedStructures: number; calibrationAttempts: number; }
export interface PuzzleGenerationTrace { stageCount: number; resolvedAt?: PuzzleGenerationDiagnosticContext; stages: readonly PuzzleGenerationStageStats[]; diagnostics: readonly PuzzleGenerationDiagnostic[]; }
export interface PuzzleGenerationMetadata { seed: number; generatorVersion: string; balanceVersion?: string; difficulty: "EASY" | "NORMAL" | "HARD" | "EXPERT"; estimatedMinimumSolutionLength: number; /** One-use operators physically placed in the generated gear. */ specialOperators?: readonly SpecialPuzzleOperator[]; /** Canonical replayed solution; this is the official three-star impulse target. */ officialSolutionImpulses?: number; officialSolutionRotationSteps?: number; branchingFactor: number; featureFlags: readonly string[]; generationStats?: { elapsedMs: number; /** Actual candidate boards evaluated before the accepted solution. */ structureAttempts: number; /** Actual replay/calibration attempts evaluated before the accepted solution. */ solutionAttempts?: number; /** Legacy name retained so historical catalogues remain readable. */ calibrationAttempts: number; /** Configured board-seed budget for one fallback stage. */ structureAttemptsBeforeScaling?: number; /** Configured replay budget for one candidate board. */ solutionAttemptsBeforeScaling?: number; totalComplexity: number; failureReasons: readonly string[]; /** Detailed v009-only trace used to calibrate catalogue generation. */ trace?: PuzzleGenerationTrace; }; }
...
export interface PersistentLevelDefinition extends BaseLevelDefinition { variant: "persistent"; innerValues: PuzzleOperator[]; /** Optional elemental affinity for each numeric gear slot. */ innerElements?: readonly (ElementalAffinity | null)[]; /** Present on authored Adventure levels; optional for isolated legacy fixtures. */ adventure?: AdventureGameConfig; }
export interface LoaderLevelDefinition extends BaseLevelDefinition { variant: "loader"; queues: PuzzleOperator[][]; /** Optional elemental affinity aligned with every queue entry. */ queueElements?: readonly (readonly (ElementalAffinity | null)[])[]; }
...
export interface AlignmentPreview { slot: PuzzleSlot; innerIndex: number; innerValue: PuzzleOperator | null; innerElement: ElementalAffinity | null; outerValue: number; result: number; active: boolean; rejectedReason?: OperationRejectedReason; trend: "zero" | "closer" | "same" | "farther"; }
