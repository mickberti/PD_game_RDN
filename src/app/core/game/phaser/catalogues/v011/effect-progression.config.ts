...
const selectionNamespace = "rdn-v011-effect-selection";
...
const areaFamilies: readonly (readonly EffectPresetKey[])[] = [
  ["AREA_BOMB_MINUS_2", "AREA_BOMB_PLUS_2", "AREA_BOMB_MINUS_4", "AREA_BOMB_PLUS_4", "AREA_BOMB_MINUS_7", "AREA_BOMB_PLUS_7"],
  ["AREA_ICE_ADJACENT", "AREA_ICE_TWO_ADJACENT", "AREA_ICE_ALL"],
  ["AREA_INVERTER_ADJACENT", "AREA_INVERTER_TWO_ADJACENT", "AREA_INVERTER_ALL"],
];
/** One family per area slot before repeating: bomb, ice and inverter have equal weight. */
const pickAreaPreset = (context: string, index: number): EffectPresetKey => {
  const family = shuffled(areaFamilies, `${context}:families`)[index % areaFamilies.length];
  return pick(family, `${context}:family:${index}`, Math.floor(index / areaFamilies.length));
};
const orderedAreaPresets = (context: string): readonly EffectPresetKey[] => {
  const families = shuffled(areaFamilies, `${context}:families`);
  return families.flatMap((family, index) => shuffled(family, `${context}:family:${index}`));
};

const areaFamilyFor = (preset: EffectPresetKey): number =>
  areaFamilies.findIndex((family) => family.includes(preset));

/**
 * High-sphere boards must not collapse into an all-Echo topology when the
 * richer candidate needs to be made safe.  Every board keeps one transform
 * link; every third level additionally reserves a Chain as its first link.
 * The cadence is tied to the visible level rather than a retry seed, so a
 * successful Chain cannot disappear merely because generation retries it.
 */
const reservesChainLink = (level: number, spheres: number): boolean =>
  level >= 100 && spheres >= 7 && (level + spheres) % 3 === 0;

const safeLinkPreset = (level: number, spheres: number, linkCount: number, index: number): EffectPresetKey => {
  if (level < 100 || spheres < 7) return "ECHO_LINK";
  if (index === 0) return reservesChainLink(level, spheres) ? "CHAIN_LINK" : "DOUBLE_LINK";
  // A Chain stays isolated from transforms so it remains compatible with two
  // special operators. Four-link boards without Chain retain a second
  // transform, preventing their fallback from becoming Echo-only.
  if (index === 1 && !reservesChainLink(level, spheres) && linkCount >= 4) return "INVERT_LINK";
  return "ECHO_LINK";
};
...
export const createProgressionEffectConfiguration = (mode: EffectProgressionMode, level: number, gemCount: number, seed = 0): LevelEffectConfiguration | undefined => {
  ...
  for (let index = 0; index < areaEffectCount; index += 1) effects.push({ preset: pickAreaPreset(selectionContext(mode, level, gemCount, seed, "area"), index), target: { type: EffectScope.AREA, sourceGemIndex: positiveModulo(second + index, gemCount) } });
...
export const withProgressionPresetVariation = (configuration: LevelEffectConfiguration, mode: EffectProgressionMode, level: number, seed: number): LevelEffectConfiguration => {
  ...
  return {
    ...
    effects: (configuration.effects ?? []).map((effect) => {
      ...
      if (effect.target.type === EffectScope.AREA) return { ...effect, preset: pickAreaPreset(selectionContext(mode, level, 0, seed, "variation-area"), areaIndex++) };
      return effect;
    }),
  };
};
...
export const policyCompatibleEffectConfiguration = (configuration: LevelEffectConfiguration, mode: EffectProgressionMode, level: number, spheres: number, seed: number, specialOperators: readonly SpecialPuzzleOperator[], enforceGemVariety = true, allowEnrichment = true): LevelEffectConfiguration => {
  ...
  let linkIndex = 0;
  let areaIndex = 0;
  const linkCount = effects.filter((effect) => effect.target.type === EffectScope.LINK).length;
  const safe = effects.map((effect) => {
    ...
    if (effect.target.type === EffectScope.LINK) return { ...effect, preset: safeLinkPreset(level, spheres, linkCount, linkIndex++), overrides: { ...effect.overrides, direction: LinkDirection.FORWARD } };
  ...
  const selected = [...safe];
  const priority = (effect: typeof effects[number]): number => effect.target.type === EffectScope.AREA ? 0 : effect.target.type === EffectScope.LINK ? 1 : 2;
  effects.map((effect, index) => ({ effect, index })).sort((left, right) => priority(left.effect) - priority(right.effect) || left.index - right.index).forEach(({ effect, index }) => {
    const scope = effect.target.type === EffectScope.GEM ? "gem" : effect.target.type === EffectScope.LINK ? "link" : "area";
    const pool = scope === "gem" ? shuffled(RDN_GEM_EFFECT_PRESETS[tier.id], selectionContext(mode, level, spheres, seed, `compatible-${scope}-${index}`)) : scope === "link" ? shuffled(level >= 100 ? RDN_LINK_EFFECT_PRESETS : RDN_LINK_EFFECT_PRESETS.filter((preset) => preset !== "CHAIN_LINK"), selectionContext(mode, level, spheres, seed, `compatible-${scope}-${index}`)) : orderedAreaPresets(selectionContext(mode, level, spheres, seed, `compatible-${scope}-${index}`));
    const usedPresets = new Set(selected.filter((current, currentIndex) => currentIndex !== index && (scope === "gem" ? current.target.type === EffectScope.GEM : scope === "link" ? current.target.type === EffectScope.LINK : current.target.type === EffectScope.AREA)).map((current) => current.preset));
    const usedAreaFamilies = new Set(selected.filter((current, currentIndex) => currentIndex !== index && current.target.type === EffectScope.AREA).map((current) => areaFamilyFor(current.preset)));
    const preferredPool = [...pool].sort((left, right) => Number(scope === "area" ? usedAreaFamilies.has(areaFamilyFor(left)) : usedPresets.has(left)) - Number(scope === "area" ? usedAreaFamilies.has(areaFamilyFor(right)) : usedPresets.has(right)));
    for (const preset of preferredPool) {
