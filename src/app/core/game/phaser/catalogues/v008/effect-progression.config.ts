...
export const withProgressionPresetVariation = (configuration: LevelEffectConfiguration, mode: EffectProgressionMode, level: number, seed: number): LevelEffectConfiguration => {
  ...
  return {
    ...
    effects: (configuration.effects ?? []).map((effect) => {
      if (effect.target.type === EffectScope.GEM) return { ...effect, preset: pick(gemPool, seed + gemIndex++ * 17) };
      if (effect.target.type === EffectScope.LINK) return { ...effect, preset: pick(linkPool, seed + linkIndex++ * 19) };
      if (effect.target.type === EffectScope.AREA) return { ...effect, preset: pick(RDN_AREA_EFFECT_PRESETS, seed + areaIndex++ * 23) };
      return effect;
    }),
  };
};
