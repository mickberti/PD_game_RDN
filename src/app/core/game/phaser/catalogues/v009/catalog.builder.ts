import { AdventureGameConfig, DEFAULT_PUZZLE_NUMBER_RANGE, LevelDefinition, PuzzleDifficulty, PuzzleGenerationDiagnosticContext, PuzzleGenerationTrace, PuzzleOperator, PuzzleSlotSolution, PuzzleSolutionMove } from "../../puzzle.types";
...
import { createFreeModeEffectConfiguration, createProgressionEffectConfiguration, policyCompatibleEffectConfiguration, validateEffectComplexity, withProgressionPresetVariation } from "./effect-progression.config";
...
type DiagnosticReporter = (code: string) => void;
...
const effectConfigurationStages = (configuration: LevelEffectConfiguration, spheres: number, mode: "adventure" | "time-attack"): readonly (readonly LevelEffectConfiguration[])[] => {
  ...
  const final = RDN_GEM_EFFECT_FALLBACK_PRESETS.map((_, offset) => ({ ...configuration, effects: minimum.map((effect, index) => effect.target.type === EffectScope.GEM ? { ...effect, preset: RDN_GEM_EFFECT_FALLBACK_PRESETS[(offset + index) % RDN_GEM_EFFECT_FALLBACK_PRESETS.length], overrides: undefined } : effect) }));
  const unique = (items: readonly LevelEffectConfiguration[]): readonly LevelEffectConfiguration[] => {
    const seen = new Set<string>();
    return items.filter((item) => { const key = JSON.stringify(item); if (seen.has(key)) return false; seen.add(key); return true; });
  };
  return [[configuration], scaled, optional, emergency, final].map(unique).filter((stage) => stage.length);
};
...
const regenerateEffectAwareLevel = <T extends LevelDefinition>(level: T, configuration: LevelEffectConfiguration | undefined, reportDiagnostic?: DiagnosticReporter): T => {
  ...
  const fail = (code: string): void => { failureReasons.add(code); reportDiagnostic?.(code); };
  const withStats = (candidate: T, solved: boolean, officialSolution?: { impulses: number; rotationSteps: number }): T => {
    if (!solved) reportDiagnostic?.("NO_VALID_EFFECT_CONFIGURATION");
    return { ...candidate, generation: candidate.generation ? { ...candidate.generation, ...(officialSolution ? { officialSolutionImpulses: officialSolution.impulses, officialSolutionRotationSteps: officialSolution.rotationSteps } : {}), generationStats: { elapsedMs: performance.now() - startedAt, structureAttempts: 1, solutionAttempts: calibrationAttempts, calibrationAttempts, structureAttemptsBeforeScaling, solutionAttemptsBeforeScaling, totalComplexity: (candidate.optimalCost?.impulses ?? 0) + (candidate.optimalCost?.rotationSteps ?? 0) + (candidate.effectConfiguration?.effects?.length ?? 0) * 10, failureReasons: solved ? [...failureReasons] : [...failureReasons, "NO_VALID_EFFECT_CONFIGURATION"] } } : candidate.generation } as T;
  };
  const range = DEFAULT_PUZZLE_NUMBER_RANGE;
  const attemptsBeforeScaling = solutionAttemptsBeforeScaling;
  if (!timerPlacementIsCompatible(level, configuration)) { fail("TIMER_TARGET_TOO_MANY_DIRECT_IMPULSES"); return withStats(level, false); }
  {
    ...
    for (let attempt = 0; attempt < attemptsBeforeScaling; attempt += 1) {
      ...
      if (seenValueVectors.has(valueVector)) { fail("CALIBRATION_VALUE_CYCLE"); break; }
      ...
      fail(timerDeadlineFailed(result) ? "TIMER_EXPIRED" : "REPLAY_NOT_WON");
      const recalculated = recalculatedOuterValues(candidate, result, range, useSignedCalibration);
      if (!recalculated) { fail("VALUES_OUT_OF_RANGE_OR_NON_INTEGER"); break; }
      if (recalculated.every((value, index) => value === outerValues[index])) { fail("VALUES_NO_LONGER_CHANGE"); break; }
...
/** The v009 sphere/mode table is canonical; obsolete checkpoints never override it. */
const applyProgressionEffects = <T extends LevelDefinition>(mode: "adventure" | "time-attack", level: T, configuration: LevelEffectConfiguration | undefined, reportDiagnostic?: DiagnosticReporter): T => regenerateEffectAwareLevel(level, configuration ?? createProgressionEffectConfiguration(mode, level.number, level.positions, level.generation?.seed ?? level.number), reportDiagnostic);
...
const effectAwareVariant = <T extends LevelDefinition>(number: number, mode: "adventure" | "time-attack", build: (variation: number) => T): T => {
  ...
  console.info(`[RDN][${mode} ${number}] avvio: ${first.positions} sfere; strutture ${attempts}; soluzioni/candidato ${solutionAttemptsBeforeScaling}; effetti richiesti ${effectSummary}.`);
  ...
  const stageStats = stages.map((stage, index) => ({ stage: index + 1, configurations: stage.length, placementVariants: 0, candidateVariants: 0, policyRejectedCandidates: 0, evaluatedStructures: 0, calibrationAttempts: 0 }));
  const diagnostics = new Map<string, { count: number; first: PuzzleGenerationDiagnosticContext; last: PuzzleGenerationDiagnosticContext }>();
  let diagnosticContext: PuzzleGenerationDiagnosticContext | undefined;
  const reportDiagnostic: DiagnosticReporter = (code) => {
    failureReasons.add(code);
    if (!diagnosticContext) return;
    const previous = diagnostics.get(code);
    diagnostics.set(code, previous
      ? { ...previous, count: previous.count + 1, last: diagnosticContext }
      : { count: 1, first: diagnosticContext, last: diagnosticContext });
  };
  const trace = (resolvedAt?: PuzzleGenerationDiagnosticContext): PuzzleGenerationTrace => ({
    stageCount: stages.length,
    ...(resolvedAt ? { resolvedAt } : {}),
    stages: stageStats,
    diagnostics: [...diagnostics.entries()].map(([code, value]) => ({ code, ...value })),
  });
  for (const [stageIndex, stage] of stages.entries()) {
    console.info(`[RDN][${mode} ${number}] stadio effetti ${stageIndex + 1}/${stages.length}: ${stage.length} configurazioni candidate.`);
    for (const [configurationIndex, configuration] of stage.entries()) {
      const placements = effectPlacementVariants(configuration, first.positions, explicitConfiguration !== undefined);
      stageStats[stageIndex].placementVariants += placements.length;
      for (const [placementIndex, positionedConfiguration] of placements.entries()) {
        for (let variation = 0; variation < attempts; variation += 1) {
          const variedConfiguration = stageIndex === 0 && explicitConfiguration === undefined
          ...
          const candidateConfiguration = policyCompatibleEffectConfiguration(
            variedConfiguration,
            mode,
            number,
            unconfiguredCandidate.positions,
            number + variation * 97,
            unconfiguredCandidate.generation?.specialOperators ?? [],
            stageIndex === 0,
            stageIndex === 0,
          );
          const context: PuzzleGenerationDiagnosticContext = {
            phase: "combination-filter",
            stage: stageIndex + 1,
          ...
          stageStats[stageIndex].candidateVariants += 1;
          ...
          if (combinationIssues.length) {
            stageStats[stageIndex].policyRejectedCandidates += 1;
            combinationIssues.forEach(reportDiagnostic);
            continue;
          }
          totalStructureAttempts += 1;
          stageStats[stageIndex].evaluatedStructures += 1;
          diagnosticContext = { ...context, phase: "calibration" };
          const candidate = applyProgressionEffects(mode, unconfiguredCandidate, candidateConfiguration, reportDiagnostic);
          last = candidate;
          const stats = candidate.generation?.generationStats;
          const calibrationAttempts = stats?.solutionAttempts ?? stats?.calibrationAttempts ?? 0;
          totalSolutionAttempts += calibrationAttempts;
          stageStats[stageIndex].calibrationAttempts += calibrationAttempts;
  ...
  reportDiagnostic("ALTERNATE_STRUCTURE_ATTEMPTS_EXHAUSTED");
...
const generateRdnLevelCatalogue = (): readonly LevelDefinition[] => {
  ...
  console.info(`[RDN] Generazione v009: livelli ${firstLevel}-${lastLevel}; modalitÃ  ${modes.join(", ")}; totale ${total}.`);
