...
/**
 * Canonical v009 board configuration. Edit this table to calibrate the two
...
export const rdnSpecialOperatorsForBoard = (level: number, spheres: number, mode: RdnCatalogueEffectMode, variation = 0): readonly Exclude<PuzzleOperator, number>[] => {
  ...
  const start = catalogueHash(`rdn-v009:special:${mode}:${level}:${spheres}:${variation}`) % RDN_SPECIAL_OPERATOR_CANDIDATES.length;
