...
import { RDN_EFFECT_SIMPLIFICATIONS, RDN_GEM_EFFECT_FALLBACK_PRESETS, rdnEffectRuleForLevel, rdnElementalAffinitiesForBoard, rdnGenerationAttemptsForSpheres, rdnProgressionRuleForSpheres, rdnSpecialOperatorsForBoard, RdnProgressionRule } from "./progression-rules.config";
...
const generationAttemptsForLevel = (level: Pick<LevelDefinition, "number" | "positions">): Pick<RdnProgressionRule, "solutionAttemptsBeforeScaling" | "structureAttemptsBeforeScaling"> => level.number > 100 ? rdnGenerationAttemptsForSpheres(level.positions) : rdnEffectRuleForLevel(level.number);
