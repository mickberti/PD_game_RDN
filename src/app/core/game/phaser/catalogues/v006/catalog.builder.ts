...
const effectConfigurationStages = (configuration: LevelEffectConfiguration, spheres: number): readonly (readonly LevelEffectConfiguration[])[] => {
  ...
  const stabilizedEffects = scaledEffects.map((effect) => {
    ...
    return preset ? { ...effect, preset, overrides: undefined } : effect;
  });
  const stabilized: LevelEffectConfiguration[] = stabilizedEffects.some((effect, index) => effect.preset !== scaledEffects[index].preset) ? [{ ...configuration, effects: stabilizedEffects }] : [];
