...
export class PuzzleEngine {
  ...
  attemptOperation(level: LevelDefinition, outerIndex: number, value: number, operator: PuzzleOperator | null, specialAlreadyConsumed = false): OperationAttemptResult {
    ...
    const nextValue = operator === "divide2" ? value / 2 : operator === "divide3" ? value / 3 : operator === "zero" ? 0 : operator === "invert" ? -value : operator === "skip" ? value : value + operator; const range = DEFAULT_PUZZLE_NUMBER_RANGE;
