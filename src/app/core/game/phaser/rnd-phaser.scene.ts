...
import { RDN_BOARD_LAYOUTS, RdnBoardLayout, getRdnBoardLayout, rdnGearTextureKey, rdnRingTextureKey } from "./rnd-board-layout.config";

export interface RdnSceneModel { level: LevelDefinition; state: PuzzleState; previews: AlignmentPreview[]; nextPreviews: AlignmentPreview[]; outcome: "win" | "lose" | null; timeRemaining: number | null; }
...
export class RdnPhaserScene extends Phaser.Scene {
  ...
  render(): void {
    ...
    if (m.timeRemaining !== null) { this.label(cx - width * .04, 26, "TEMPO", 10, 0xf3d27c); this.label(cx - width * .04, 53, this.formatTime(m.timeRemaining), 22, 0xf3d27c); }
    this.label(cx + width * .12, 26, "IMPULSI", 10, 0xf3d27c); this.label(cx + width * .12, 53, String(m.state.impulses), 27, 0xf3d27c);
    this.label(cx + width * .29, 26, "ROT.", 10, 0xf3d27c); this.label(cx + width * .29, 53, String(m.state.rotationSteps), 27, 0xf3d27c);
    ...
    this.bonus(cx - 95, height - 50, "action-heal", "3"); this.bonus(cx, height - 50, "action-defense", "3"); this.bonus(cx + 95, height - 50, "action-holy-star", "3");
    if (m.outcome) this.dialog(cx, cy, m.outcome, m);
  }
  private beginDrag(pointer: Phaser.Input.Pointer): void { if (this.busy || !this.model || this.model.outcome) return; const dx = pointer.x - this.wheelCenter.x; const dy = pointer.y - this.wheelCenter.y; const distance = Math.hypot(dx, dy); if (distance > this.wheelCenter.radius || distance < this.wheelCenter.radius * .3) return; this.dragging = true; this.dragStart = Math.atan2(dy, dx); this.dragDelta = 0; }
  ...
  private drawConnections(ringX: number, ringY: number, sourceX: number, sourceY: number, radius: number, model: RdnSceneModel): void {
    ...
    for (const preview of active) this.drawTrail(ringX, ringY, sourceX, sourceY, radius, preview.slot.outerIndex, model.level.positions, 0x59e77c, .9);
    for (const preview of model.nextPreviews) if (preview.active && !activeIndexes.has(preview.slot.outerIndex)) this.drawTrail(ringX, ringY, sourceX, sourceY, radius, preview.slot.outerIndex, model.level.positions, 0xe9c14d, .65);
  }
  /** A curved conduit from the impulse core that coils once around the target sphere. */
  private drawTrail(ringX: number, ringY: number, sourceX: number, sourceY: number, radius: number, outerIndex: number, total: number, color: number, alpha: number): void {
    ...
    const pulseGlow = trace(radius * config.glowWidth, alpha * .15).setAlpha(.28); this.trailEffects.push(pulseGlow); this.tweens.add({ targets: pulseGlow, alpha: .92, duration: 1050, ease: "Sine.InOut", yoyo: true, repeat: -1 });
    trace(radius * config.middleWidth, alpha * .12); trace(radius * config.coreWidth, alpha * .43);
  ...
  private dialog(cx: number, cy: number, outcome: "win" | "lose", model: RdnSceneModel): void { const depth = 20; this.add.rectangle(cx, cy, 310, 180, 0x151914, .96).setStrokeStyle(3, 0xc49b50).setDepth(depth); this.label(cx, cy - 50, outcome === "win" ? "LIVELLO COMPLETATO" : "TENTATIVO FALLITO", 19, 0xf8dc8b).setDepth(depth + 1); if (outcome === "win") { const stars = this.stars(model); this.label(cx, cy - 15, "★".repeat(stars) + "☆".repeat(3 - stars), 27, 0xffdf70).setDepth(depth + 1); this.button(cx - 62, cy + 48, "▶", () => this.actions.continue(), depth + 2); this.label(cx - 62, cy + 80, "PROSEGUI", 10, 0xf8dc8b).setDepth(depth + 4); } else { this.button(cx - 62, cy + 48, "↻", () => this.actions.retry(), depth + 2); this.label(cx - 62, cy + 80, "RITENTA", 10, 0xf8dc8b).setDepth(depth + 4); } this.button(cx + 62, cy + 48, "×", () => this.actions.exit(), depth + 2); this.label(cx + 62, cy + 80, "ESCI", 10, 0xf8dc8b).setDepth(depth + 4); }
  private stars(model: RdnSceneModel): number { const cost = model.level.optimalCost; if (!cost) return 1; if (model.state.impulses <= cost.impulses && model.state.rotationSteps <= cost.rotationSteps) return 3; return model.state.impulses <= cost.impulses + 2 ? 2 : 1; }
}
