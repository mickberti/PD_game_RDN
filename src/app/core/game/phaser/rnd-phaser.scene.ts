...
const formatOperator = (value: PuzzleOperator | null): string => value === null ? "—" : value === "divide2" ? "÷2" : value > 0 ? `+${value}` : String(value);
...
export class RdnPhaserScene extends Phaser.Scene {
  ...
  render(): void {
    ...
    for (let index = 0; index < m.level.positions; index += 1) { const point = this.point(0, 0, wheelR * layout.innerSlots.radius, index, m.level.positions, layout.innerSlots.angleOffset); const innerIndex = index; const queue = m.level.variant === "loader" ? m.queueStates[innerIndex] : undefined; const rawValue = m.level.variant === "persistent" ? m.level.innerValues[innerIndex] : queue?.current ?? null; const consumed = rawValue === "divide2" && m.state.consumedSpecialOperatorIndexes.includes(innerIndex); const exhausted = queue?.exhausted ?? false; const value = consumed || exhausted ? null : rawValue; const sphereRadius = outerR * layout.innerSlots.sphereRadius; const blocked = blockedSources.has(innerIndex); const deactivated = blocked || consumed || exhausted; const gem = this.gem(gearX + point.x, gearY + point.y, sphereRadius, index, visualSet, deactivated, rawValue === "divide2").setDepth(4); const text = this.label(gearX + point.x, gearY + point.y, consumed || exhausted ? "" : formatOperator(value), Math.round(sphereRadius * .76), deactivated ? 0xd8d8d8 : 0xffffff).setDepth(5); const badge = queue ? this.queueBadge(gearX + point.x + sphereRadius * .72, gearY + point.y + sphereRadius * .72, queue.remainingCount, exhausted) : undefined; this.innerSlots.push({ sphere: gem, text, badge, localX: point.x, localY: point.y }); }
  ...
  private drawConnections(ringX: number, ringY: number, sourceX: number, sourceY: number, radius: number, model: RdnSceneModel): void {
    ...
    for (const flow of model.flows) this.drawTrail(ringX, ringY, sourceX, sourceY, radius, flow.targetId, model.level.positions, flow.interactable ? 0x59e77c : 0x929292, flow.interactable ? .95 : .76, 1.32, flow.interactable);
    ...
    for (const preview of model.nextPreviews) if (!currentTargets.has(preview.slot.outerIndex)) this.drawTrail(ringX, ringY, sourceX, sourceY, radius, preview.slot.outerIndex, model.level.positions, 0xf3ce55, .68, 1.1, true);
  }
  ...
  private sphere(x: number, y: number, radius: number, value: number, off: boolean, index: number, visualSet: number): void { this.gem(x, y, radius, index, visualSet, off); this.label(x, y, off ? "0" : format(value), Math.max(14, Math.round(radius * .72)), off ? 0xe6e6e6 : 0xffffff); }
  ...
  private operationFeedback(reason: AlignmentPreview["rejectedReason"]): string { return reason === "DIVIDE_BY_TWO_REQUIRES_NON_ZERO_EVEN_INTEGER" ? "DIV2 richiede un valore pari diverso da zero" : reason === "DIVIDE_BY_TWO_CONSUMED" ? "DIV2 gia usato" : reason === "RESULT_OUT_OF_RANGE" ? "Mossa fuori intervallo" : "Mossa non disponibile"; }
  ...
  private label(x: number, y: number, value: string, size: number, color: number): Phaser.GameObjects.Text { return this.add.text(x, y, value, { fontFamily: "Arial", fontSize: `${size}px`, color: `#${color.toString(16).padStart(6, "0")}`, fontStyle: "bold", stroke: "#111814", strokeThickness: Math.max(2, Math.round(size * .14)) }).setOrigin(.5); }
  ...
  private bonus(x: number, y: number, action: RdnHudAction, activate: () => void): void { const button = this.add.circle(x, y, 31, action.disabled ? 0x4d4d4d : 0x245438).setStrokeStyle(3, action.disabled ? 0x888888 : 0xb18b48).setInteractive(); button.on("pointerdown", () => { if (!action.disabled) activate(); }); this.actionIcon(x, y, 75, action.icon, 3); this.add.circle(x + 23, y + 23, 12, 0x241b12).setStrokeStyle(1, 0xb18b48).setDepth(4); this.label(x + 23, y + 23, String(action.charges), 12, 0xffffff).setDepth(5); }
  private infoDialog(cx: number, cy: number, model: RdnSceneModel): void {
    const depth = 20;
    this.add.rectangle(cx, cy, 330, 238, 0x151914, .97).setStrokeStyle(3, 0xc49b50).setDepth(depth).setInteractive();
    if (model.level.variant === "persistent") {
      const config = model.level.adventure;
      const div2 = config?.specialInventory.divide2 ?? 0;
      const limits = config?.limits;
      this.label(cx, cy - 82, "RISORSE ADVENTURE", 18, 0xf8dc8b).setDepth(depth + 1);
      this.label(cx, cy - 43, "Operatori numerici: fissi e infiniti", 13, 0xe6dfc3).setDepth(depth + 1);
      this.label(cx, cy - 14, `DIV2: ${Math.max(0, div2 - model.state.consumedSpecialOperatorIndexes.length)}/${div2} monouso`, 14, 0xffdf70).setDepth(depth + 1);
      this.label(cx, cy + 17, "Obiettivo: tutte le gemme a zero", 13, 0xe6dfc3).setDepth(depth + 1);
      this.label(cx, cy + 46, limits?.maxImpulses !== undefined ? `Limite: ${limits.maxImpulses} impulsi` : "Nessun limite di sconfitta", 12, limits?.maxImpulses !== undefined ? 0xfb8c8c : 0x9df3a8).setDepth(depth + 1);
    } else {
    ...
    this.button(cx, cy + 88, "×", () => this.actions.closeInfo(), depth + 2);
  }
