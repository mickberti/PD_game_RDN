...
export interface RdnSceneModel { level: LevelDefinition; state: PuzzleState; previews: AlignmentPreview[]; nextPreviews: AlignmentPreview[]; flows: FlowState[]; effectPreviewEvents: readonly EffectEngineEvent[]; queueStates: readonly QueueState[]; actions: readonly RdnHudAction[]; modeLabel: string; freeSettings?: { difficulty: "EASY" | "NORMAL" | "HARD" | "EXPERT"; slotCount: number; effectsEnabled: boolean; }; playground?: { scenario: string; index: number; total: number; lines: readonly string[] }; selectedGemIndex: number | null; selectedLinkEffectId: string | null; outcome: "win" | "lose" | null; timeRemaining: number | null; timeRemainingMs?: number | null; timeTotalSeconds?: number; showInfo: boolean; }
...
export class RdnPhaserScene extends Phaser.Scene {
  ...
  private isCountdownOnlyUpdate(next: RdnSceneModel): boolean {
    ...
    return !!previous && previous.timeRemaining !== null && next.timeRemaining !== null && previous.level === next.level && previous.state === next.state && previous.outcome === next.outcome && previous.showInfo === next.showInfo && previous.selectedGemIndex === next.selectedGemIndex && previous.selectedLinkEffectId === next.selectedLinkEffectId && previous.modeLabel === next.modeLabel && previous.timeTotalSeconds === next.timeTotalSeconds;
  }
