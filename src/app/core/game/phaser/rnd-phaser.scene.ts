...
export class RdnPhaserScene extends Phaser.Scene {
  ...
  private infoDialog(cx: number, cy: number, model: RdnSceneModel): void {
    const depth = 20;
    this.add.rectangle(cx, cy, 330, model.level.variant === "persistent" ? 278 : 238, 0x151914, .97).setStrokeStyle(3, 0xc49b50).setDepth(depth).setInteractive();
    if (model.level.variant === "persistent") {
      ...
      this.label(cx, cy + 65, limits?.maxImpulses !== undefined ? `Limite: ${limits.maxImpulses} impulsi` : "Nessun limite di sconfitta", 11, limits?.maxImpulses !== undefined ? 0xfb8c8c : 0x9df3a8).setDepth(depth + 1);
    } else {
      const policy = getPuzzleScorePolicy(model.level);
