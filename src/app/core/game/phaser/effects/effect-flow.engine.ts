import { AreaEffectConfig, AreaEffectRange, AreaEffectType, DEFAULT_FLOW_RULES, EffectEngineEvent, EffectRuntimeState, EffectScope, ElementalAffinity, FlowEvent, FlowRules, GemEffectConfig, GemEffectType, LinkDirection, LinkEffectConfig, LinkEffectType, ResolvedEffect } from "./effects.models";

export interface EffectFlowInput { gemId: string; value: number; elementalAffinity?: ElementalAffinity; }
...
export class EffectFlowEngine {
  createRuntime(effects: readonly ResolvedEffect[]): EffectRuntimeState {
    const wallRemainingStrength: Record<string, number> = {}; const iceRemainingStrength: Record<string, number> = {}; const fireRemainingStrength: Record<string, number> = {}; const shieldRemainingStrength: Record<string, number> = {}; const timerRemainingTurns: Record<string, number> = {};
    for (const effect of effects) if (effect.config.scope === EffectScope.GEM) {
      ...
      if (config.type === GemEffectType.FIRE) fireRemainingStrength[effect.id] = config.strength;
  ...
  resolve(values: readonly number[], effects: readonly ResolvedEffect[], runtime: EffectRuntimeState, inputs: readonly EffectFlowInput[], overrides?: Partial<FlowRules>, turn?: number, resolvedGemIds: readonly string[] = []): EffectFlowResult {
    ...
    const nextRuntime: EffectRuntimeState = { wallRemainingStrength: { ...(runtime.wallRemainingStrength ?? {}) }, iceRemainingStrength: { ...(runtime.iceRemainingStrength ?? {}) }, fireRemainingStrength: { ...(runtime.fireRemainingStrength ?? {}) }, areaIceRemainingStrength: { ...(runtime.areaIceRemainingStrength ?? {}) }, shieldRemainingStrength: { ...(runtime.shieldRemainingStrength ?? {}) }, timerRemainingTurns: { ...(runtime.timerRemainingTurns ?? {}) }, completedTimerIds: [...(runtime.completedTimerIds ?? [])], expiredTimerIds: [...(runtime.expiredTimerIds ?? [])], turn: turn ?? (runtime.turn ?? 0) + 1 };
    ...
    const gemEffects = new Map<string, ResolvedEffect[]>(); const links = effects.filter((effect): effect is ResolvedEffect & { config: LinkEffectConfig } => effect.config.scope === EffectScope.LINK); const chainLinks = links.filter((effect) => effect.config.type === LinkEffectType.CHAIN); const areaEffects = effects.filter((effect): effect is ResolvedEffect & { config: AreaEffectConfig } => effect.config.scope === EffectScope.AREA);
    ...
    let queue: FlowEvent[] = inputs.map((input, index) => ({ id: `flow-${index}`, rootFlowId: `flow-${index}`, originGemId: input.gemId, currentGemId: input.gemId, value: input.value, elementalAffinity: input.elementalAffinity, generation: 0, sourceType: "DIRECT", visitedLinks: new Set<string>() }));
    ...
    while (queue.length) {
      ...
      for (const flow of batch) {
        ...
        const processed = chainedBy ? (events.push({ type: "CHAIN_BLOCKED", flowId: flow.id, gemId: flow.currentGemId, linkId: chainedBy.id, generation }), { flow, value: 0, blocked: true }) : this.applyBeforeEffects(gemEffects.get(flow.currentGemId) ?? [], flow.value, nextRuntime, events, flow); events.push({ type: "FLOW_ARRIVED", flowId: flow.id, gemId: flow.currentGemId, value: processed.value, generation });
      ...
      for (const arrival of arrivals) {
        ...
        for (const link of links) { if (link.config.type === LinkEffectType.CHAIN) continue; const target = this.followLink(link, arrival.flow.currentGemId); if (!target || !gemIndex.has(target) || terminalGems.has(target) || arrival.flow.visitedLinks.has(link.id)) continue; const propagated = this.transformLink(link.config, arrival.value); const visitedLinks = new Set(arrival.flow.visitedLinks); visitedLinks.add(link.id); const id = `${arrival.flow.id}>${link.id}`; queue.push({ id, rootFlowId: arrival.flow.rootFlowId, originGemId: arrival.flow.originGemId, currentGemId: target, value: propagated, elementalAffinity: arrival.flow.elementalAffinity, generation: generation + 1, sourceType: "PROPAGATED", visitedLinks }); events.push({ type: "FLOW_PROPAGATED", flowId: id, gemId: target, linkId: link.id, value: propagated, elementalAffinity: arrival.flow.elementalAffinity, generation: generation + 1 }); }
      }
  ...
  private applyBeforeEffects(effects: readonly ResolvedEffect[], incoming: number, runtime: EffectRuntimeState, events: EffectEngineEvent[], flow: FlowEvent): ProcessedContribution {
    ...
    for (const effect of effects) {
      ...
      if (config.type === GemEffectType.WALL || config.type === GemEffectType.ICE || config.type === GemEffectType.FIRE) {
        const barrierAffinity: ElementalAffinity | null = config.type === GemEffectType.ICE ? "ice" : config.type === GemEffectType.FIRE ? "fire" : null;
        const state = config.type === GemEffectType.WALL ? runtime.wallRemainingStrength : config.type === GemEffectType.ICE ? runtime.iceRemainingStrength : runtime.fireRemainingStrength;
        const remaining = state[effect.id] ?? 0;
        if (remaining > 0) {
          const opposite = barrierAffinity === "ice" ? "fire" : barrierAffinity === "fire" ? "ice" : null;
          if (opposite && flow.elementalAffinity === opposite) { events.push({ type: "ELEMENTAL_BYPASSED", flowId: flow.id, gemId: flow.currentGemId, generation: flow.generation, elementalAffinity: barrierAffinity ?? undefined }); continue; }
          if (barrierAffinity && flow.elementalAffinity === barrierAffinity) { events.push({ type: "ELEMENTAL_BLOCKED", flowId: flow.id, gemId: flow.currentGemId, generation: flow.generation, elementalAffinity: barrierAffinity, remainingStrength: remaining, initialStrength: config.strength }); return { flow, value: 0, blocked: true }; }
          const next = remaining - 1;
          if (config.type === GemEffectType.WALL) runtime.wallRemainingStrength = { ...runtime.wallRemainingStrength, [effect.id]: next };
          else if (config.type === GemEffectType.ICE) runtime.iceRemainingStrength = { ...runtime.iceRemainingStrength, [effect.id]: next };
          else runtime.fireRemainingStrength = { ...runtime.fireRemainingStrength, [effect.id]: next };
          const hit = config.type === GemEffectType.WALL ? "WALL_HIT" : config.type === GemEffectType.ICE ? "ICE_HIT" : "FIRE_HIT";
          const broken = config.type === GemEffectType.WALL ? "WALL_BROKEN" : config.type === GemEffectType.ICE ? "ICE_BROKEN" : "FIRE_BROKEN";
  ...
  private priority(type: GemEffectType): number { return type === GemEffectType.WALL || type === GemEffectType.ICE || type === GemEffectType.FIRE ? 10 : type === GemEffectType.SHIELD ? 20 : type === GemEffectType.MIRROR ? 30 : type === GemEffectType.AMPLIFIER ? 40 : type === GemEffectType.INVERTER ? 90 : 100; }
  ...
  private chainPrerequisite(chains: readonly (ResolvedEffect & { config: LinkEffectConfig })[], gemId: string, values: readonly number[]): ResolvedEffect | undefined { return chains.find((chain) => chain.target.type === EffectScope.LINK && chain.target.toGem.id === gemId && values[chain.target.fromGem.index] !== 0); }
