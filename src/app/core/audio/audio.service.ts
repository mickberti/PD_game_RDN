...
import { AUDIO_SETTINGS, MUSIC_CONFIG, SFX_CONFIG } from "./audio.config";
import { AudioAssetStatus, AudioCue, AudioCueConfig, AudioDebugCounters, AudioDebugEvent, AudioSettings, MusicCue } from "./audio.models";

const STORAGE_KEY = "gearithm.audio.settings.v1";
const defaults: AudioSettings = { masterVolume: 1, musicVolume: AUDIO_SETTINGS.music.defaultVolume, sfxVolume: AUDIO_SETTINGS.sfx.defaultVolume, masterMuted: false, musicMuted: false, sfxMuted: false };
...
export class AudioService {
  ...
  playMusic(cue: MusicCue, crossfadeMs = AUDIO_SETTINGS.music.crossfadeMs): void {
    if (this.currentMusic?.cue === cue) return;
    ...
    void next.play().catch(() => this.warn(`Unable to play music ${cue}`)); this.fade(next, this.finalMusicVolume(MUSIC_CONFIG[cue]), crossfadeMs);
  ...
  resumeMusic(): void { if (this.currentMusic) void this.currentMusic.element.play().catch(() => undefined); }
  ...
  resetSettings(): void { this.masterVolume.set(defaults.masterVolume); this.musicVolume.set(defaults.musicVolume); this.sfxVolume.set(defaults.sfxVolume); this.masterMuted.set(false); this.musicMuted.set(false); this.sfxMuted.set(false); this.persist(); this.refreshVolumes(); }
  ...
  private createAudio(config: AudioCueConfig, statusKey: string): HTMLAudioElement { const element = new Audio(); const sources = config.variants?.[Math.floor(Math.random() * config.variants.length)] ?? config.sources; element.preload = "auto"; element.src = sources[0]; element.volume = this.finalSfxVolume(config); element.playbackRate = config.pitchVariation ? 1 + (Math.random() * 2 - 1) * config.pitchVariation : 1; element.onerror = () => { this.setAssetStatus(statusKey, "MISSING"); this.warn(`Missing or undecodable audio asset: ${sources.join(", ")}`); }; return element; }
