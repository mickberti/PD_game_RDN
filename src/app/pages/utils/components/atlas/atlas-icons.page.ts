...
@Component({
  selector: 'app-atlas-gallery',
  standalone: true,
  imports: [CommonModule, IonContent, IonToolbar, IonFooter, UIBottomUtilsComponent, UiUtilsPageHeaderComponent],
  templateUrl: './atlas-icons.page.html',
  styleUrls: ['./atlas-icons.page.scss']
})
export class AtlasIconsPage {
  ...
  atlasOptions: AtlasOption[] = [];
  readonly isHeroAtlasPage: boolean;

  selectedAtlasId = '';
  gridColumns = 4;
  selectedPreviewFormatId = this.previewFormatOptions[1].id;
  showAtlasImagePreview = false;
  viewMode: AtlasViewMode = 'grid';
  ...
  constructor(
    ...
    this.isHeroAtlasPage = this.route.snapshot.data['atlasScope'] === 'heroes';
    this.atlasOptions = this.isHeroAtlasPage
      ? this.atlasService.configuredAtlasHeroOptions
      : this.atlasService.configuredAtlasOptions;
    this.selectedAtlasId = this.atlasOptions[0]?.id ?? '';
    this.reloadIcons();
  }
