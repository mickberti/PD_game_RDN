...
import { AlignmentPreview } from "../../core/game/rnd/puzzle.types";
...
@Component({
  ...
  template: `<ion-content><main class="rnd-game" [attr.data-variant]="session.variant"><section class="rnd-board" aria-label="Tabellone RDN">
      <header class="rnd-hud"><button aria-label="Impostazioni">⚙</button><strong><small>{{ level().variant === 'persistent' ? 'AVVENTURA' : 'TIME ATTACK' }}</small>{{ level().number }}</strong><div class="rnd-emblem">△</div><strong><small>ROTATIONS</small>{{ state().rotationSteps }}</strong><button aria-label="Riavvia" (click)="restart()">↻</button></header>
      <div class="rnd-outer-ring">@for (value of state().outerValues; track $index) { <div class="rnd-orbit outer" [style.transform]="orbit($index, state().outerValues.length, 46)">@if (isActive($index)) { <i class="rnd-connection active"></i> } @else if (isNextActive($index)) { <i class="rnd-connection next"></i> }<span class="rnd-sphere" [style.transform]="outerSphereTransform($index, state().outerValues.length)" [class.rnd-zero]="value === 0">{{ format(value) }}</span>@if (previewAt($index); as preview) { <small class="rnd-operation-banner" [attr.data-trend]="preview.trend" [style.transform]="outerBannerTransform($index, state().outerValues.length)">{{ format(preview.result) }} {{ trendIcon(preview) }}</small> }</div> }</div>
      <div class="rnd-wheel" [class.is-dragging]="isDragging()" [style.transform]="wheelTransform()" (pointerdown)="dragStart($event)" (pointermove)="dragMove($event)" (pointerup)="dragEnd($event)" (pointercancel)="dragCancel($event)">
        ...
        @for (value of innerValues(); track $index) { <div class="rnd-orbit inner" [style.transform]="orbit($index, innerValues().length, 34)"><span class="rnd-sphere inner-value" [style.transform]="innerSphereTransform($index, innerValues().length)" [attr.aria-label]="queueLabel($index)">{{ format(value) }}@if (level().variant === 'loader') { <small>{{ nextQueueValue($index) }}</small> }</span></div> }
      ...
      <nav class="rnd-bonuses" aria-label="Bonus disponibili"><button type="button" class="rnd-bonus potion" aria-label="Bonus pozione, 3 disponibili"><span>⚗</span><b>3</b></button><button type="button" class="rnd-bonus hammer" aria-label="Bonus martello, 3 disponibili"><span>⚒</span><b>3</b></button><button type="button" class="rnd-bonus compass" aria-label="Bonus bussola, 3 disponibili"><span>✥</span><b>3</b></button></nav>
    </section>@if (state().won) { <p class="rnd-win">Meccanismo risolto.</p> } @if (outcome(); as result) { <section class="rnd-result" role="dialog" aria-modal="true"><h2>{{ result === 'win' ? 'Livello completato' : 'Nessuna operazione disponibile' }}</h2><p>{{ result === 'win' ? 'Hai azzerato tutti i valori esterni.' : 'Riprova una rotazione o una strategia diversa.' }}</p>@if (result === 'win') { <div class="rnd-stars" [attr.aria-label]="stars() + ' stelle su 3'">{{ starDisplay() }}</div><button (click)="continue()">Prosegui</button> } @else { <button (click)="retry()">Ritenta</button> }<button (click)="exit()">Esci</button></section> }
  </main></ion-content>`,
...
export class GameplayPageComponent {
  ...
  impulse(): void { this.puzzle.dispatch({ type: "IMPULSE" }); if (this.state().won) this.outcome.set("win"); else if (!this.previews().some((preview) => preview.active)) this.outcome.set("lose"); }
  undo(): void { this.puzzle.dispatch({ type: "UNDO" }); }
  restart(): void { this.puzzle.dispatch({ type: "RESTART" }); this.outcome.set(null); }
  ...
  innerSphereTransform(index: number, total: number): string { return `translate(-50%,-50%) rotate(${-index * 360 / total - this.wheelAngle()}deg)`; }
  outerBannerTransform(index: number, total: number): string { return `translate(-50%,-50%) rotate(${-index * 360 / total}deg)`; }
  wheelAngle(): number { return this.state().rotationTurns * 360 / this.level().positions + this.dragAngle(); }
  wheelTransform(): string { return `translate(-50%,-50%) rotate(${this.wheelAngle()}deg)`; }
  ...
  nextQueueValue(index: number): string { const level = this.level(); return level.variant === "loader" ? this.format(level.queues[index][this.state().queueCursors[index] + 1] ?? null) : ""; }
  queueLabel(index: number): string { return this.level().variant === "loader" ? `Coda: ora ${this.format(this.innerValues()[index])}, poi ${this.nextQueueValue(index)}` : `Operatore ${this.format(this.innerValues()[index])}`; }
  activeCount(): number { return this.previews().filter((preview) => preview.active).length; }
  previewAt(outerIndex: number): AlignmentPreview | undefined { return this.previews().find((preview) => preview.slot.outerIndex === outerIndex); }
  isActive(outerIndex: number): boolean { return this.previews().some((preview) => preview.slot.outerIndex === outerIndex); }
  isNextActive(outerIndex: number): boolean { return !this.isActive(outerIndex) && this.nextPreviews().some((preview) => preview.slot.outerIndex === outerIndex); }
  trendIcon(preview: AlignmentPreview): string { return preview.trend === "zero" ? "✓" : preview.trend === "closer" ? "↓" : preview.trend === "farther" ? "↑" : "•"; }
  stars(): number { const cost = this.level().optimalCost; if (!cost) return 1; const impulses = this.state().impulses; return impulses <= cost.impulses && this.state().rotationSteps <= cost.rotationSteps ? 3 : impulses <= cost.impulses + 2 ? 2 : 1; }
  starDisplay(): string { return "★".repeat(this.stars()) + "☆".repeat(3 - this.stars()); }
  continue(): void { const next = Math.min(50, this.level().number + 1); this.puzzle.load(this.session.variant, next); this.outcome.set(null); }
  retry(): void { this.restart(); }
  exit(): void { void this.nav.go("/hub"); }
