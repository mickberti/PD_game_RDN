...
@Component({
  ...
  template: `<ion-content><main class="rnd-game" [attr.data-variant]="session.variant">
    ...
    </section>@if (state().won) { <p class="rnd-win">Meccanismo risolto.</p> } @if (outcome(); as result) { <section class="rnd-result" role="dialog" aria-modal="true"><h2>{{ result === 'win' ? 'Livello completato' : 'Nessuna operazione disponibile' }}</h2><p>{{ result === 'win' ? 'Hai azzerato tutti i valori esterni.' : 'Riprova una rotazione o una strategia diversa.' }}</p><button (click)="continue()">Prosegui</button><button (click)="retry()">Ritenta</button><button (click)="exit()">Esci</button></section> }
  </main></ion-content>`,
